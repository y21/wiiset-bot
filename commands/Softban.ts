...
export default <Command>{
    name: "softban",
    description: "Softbans a user from this server: bans a user and unbans them immediately after to delete all messages sent by this user in the past.",
    args: [{
        name: "user",
        description: "The user to softban. This needs to be either a mention or a user ID.",
        required: true
    }],
    guildOnly: true,
    category: "mod",
    ...
    run: async (base: Base, message: Message) => {
        if (!message.member.hasPermission("BAN_MEMBERS"))
            throw new Error("You don't have permissions to ban users which is required for the softban command.");
        ...
        const confirmMessage: Message | Message[] = await message.channel.send(`Do you really want to softban __${target.user.tag}__? Reply with either **y** or **n** in the next 15 seconds to confirm your softban.\n\nReason: \`${args.slice(2).join(" ")}\``);
        ...
        collector.on("collect", (msg: Message) => {
            ...
            if (msg.content.toLowerCase() === "y" || msg.content.toLowerCase() === "yes") {
                target.ban({
                    ...
                    if (!target) return;
                    message.guild.unban(target.id).then(() => {
                        confirmMessage.edit("Softbanned.");
                    });
                });
            } else if (msg.content.toLowerCase() === "n" || msg.content.toLowerCase() === "no") {
                confirmMessage.edit("Softban aborted.");
            }
        });
    }
}