module.exports = async message => {
	try {
		...
		message.reply((message.translations.commands.ban_confirmation || "Translation error").replace(/\{target\}/g, target.user.tag).replace(/\{reason\}/, (message.args.slice(2).join(" ") || "- none -")));
		...
		collector.on("collect", m => {
			if(m.content === "y" || m.content === "yes"){
				target.ban({ days: 7, reason: "Ban requested by " + message.author.tag + " | Reason: " + (message.args.slice(2).join(" ") || "- none -")}).then(() => {
					...
					message.reply((message.translations.commands.ban_error || "Translation error") + " `" + err.toString() + "`");
				});
			} else {
