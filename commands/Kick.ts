...
export default <Command>{
    ...
    args: [{
        name: "user",
        description: "The user to kick. This needs to be either a mention or a user ID.",
        required: true
    }],
    guildOnly: true,
    category: "mod",
    ...
    run: async (base: Base, message: Message) => {
        if (!message.member.hasPermission("KICK_MEMBERS"))
            throw new Error("You don't have permissions to kick users.");
        ...
        const confirmMessage: Message | Message[] = await message.channel.send(`Do you really want to kick __${target.user.tag}__? Reply with either **y** or **n** in the next 15 seconds to confirm your kick.\n\nReason: \`${args.slice(2).join(" ")}\``);
        ...
        collector.on("collect", (msg: Message) => {
            ...
            if (msg.content.toLowerCase() === "y" || msg.content.toLowerCase() === "yes") {
                target.kick("Kicked by " + message.author.tag + ". Reason: " + args.slice(2).join(" "))
                    .then(() => {
                    confirmMessage.edit("Kicked.");
                });
            } else if (msg.content.toLowerCase() === "n" || msg.content.toLowerCase() === "no") {
                confirmMessage.edit("Kick aborted.");
            }
        });
    }
}