...
module.exports = async message => {
    let request = await get(`https://wiimmfi.glitch.me/findUser?name=${message.content.split(" ").slice(2).join(" ").substr(0, message.content.indexOf("-flag:") > -1 ? message.content.split(" ").slice(2).join(" ").indexOf("-flag:") - 1 : message.content.split(" ").slice(2).join(" ").length).replace(/ /g, '%20')}${fromString(message.content) ? "&flags=" + fromString(message.content).join(",") : ""}`).catch(console.log);
